...
from .node_types import ContentNode, EncoderNode, NodeTypeBase
...
class NodeController:
    def __init__(self, manager_settings: ManagerSettings, definitions: CloudInstanceDefsController):
        ...
        self.dns_manager: Optional[DNSManager] = get_dns_api_by_provider(definitions)
    ...
    async def get_pending_nodes(self, node_type: Type[NodeTypeBase]) -> List[NodeTypeBase]:
    ...
    def get_operating_nodes(self, node_type: Type[NodeTypeBase]) -> List[NodeTypeBase]:
        """Get all nodes that are operating well."""
        nodes = self.nodes[node_type]
        list = []
        for node in nodes:
            # TODO also consider the current status as reported by node
            if isinstance(node.server, StaticServer) or \
                    (isinstance(node.server, DynamicServer) and DeploymentStatus.state_ok(node.server.deployment_status)):
                list.append(node)
        return list
    ...
    async def _init_static_nodes(self) -> None:
        content_node_urls = list(map(str.strip, self.manager_settings.static_content_node_base_urls.split(',')))
        for url in content_node_urls:
            url_parsed = urlparse(url)
            node = ContentNode()
            node.base_url = ensure_url_does_not_end_with_slash(url)
            node.server = StaticServer(url_parsed.netloc, url_parsed.hostname)
            self.nodes[ContentNode].append(node)
    ...
    async def _static_server_lifecycle(self, node: NodeTypeBase):
        ...
        while True:
            try:
                ...
                logger.exception(f"Error in static server lifecycle (server {node.server.name})")
                await sleep(20)
